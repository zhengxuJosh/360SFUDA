...
from .stanford_pin_dataset import __FOLD__
...
class StanfordPanDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(321, 321), 
                 ...
                 set='val', ssl_dir='/home/jinjing/zhengxu/SFUDA/data/Stanford2D3D', trans='FixScaleRandomCropWH',
                 fold=1):
        self.root = root
    ...
    def __getitem__(self, index):
        ...
        #label_name = name.replace('/rgb','/semantic')
        label = Image.open(osp.join(self.ssl_dir, name.replace('rgb','semantic')))#('.png', '_labelTrainIds.png')))
...
class StanfordPanTestDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(4096, 2048), mean=(128, 128, 128),
        ...
        self.files = []
        # --- stanford color2id
        with open('/home/jinjing/zhengxu/SFUDA/DPPASS/dataset/adaption/s2d3d_pin_list/semantic_labels.json') as f:
            id2name = [name.split('_')[0] for name in json.load(f)] + ['<UNK>']
        with open('/home/jinjing/zhengxu/SFUDA/DPPASS/dataset/adaption/s2d3d_pin_list/name2label.json') as f:
            name2id = json.load(f)
        self.colors = np.load('/home/jinjing/zhengxu/SFUDA/DPPASS/dataset/adaption/s2d3d_pin_list/colors.npy') # for visualization
