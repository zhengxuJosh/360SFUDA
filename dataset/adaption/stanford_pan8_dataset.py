...
class StanfordPan8DataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(321, 321), 
                 ...
                 set='train', ssl_dir='', trans='FixScaleRandomCropWH',
                 fold=1):
        self.root = root
...
class StanfordPan8TestDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(4096, 2048), mean=(128, 128, 128),
        ...
        self.files = []
        # --- stanford color2id
        with open('/hpc2hdd/home/xzheng287/360Seg/DPPASS/dataset/adaption/s2d3d_pin_list/semantic_labels.json') as f:
            id2name = [name.split('_')[0] for name in json.load(f)] + ['<UNK>']
        with open('/hpc2hdd/home/xzheng287/360Seg/DPPASS/dataset/adaption/s2d3d_pin_list/name2label.json') as f:
            name2id = json.load(f)
        self.colors = np.load('/hpc2hdd/home/xzheng287/360Seg/DPPASS/dataset/adaption/s2d3d_pin_list/colors.npy') # for visualization
