...
class StanfordPinDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(321, 321), mean=(128, 128, 128),
        ...
        self.files = []
        # --- stanford color2id
        with open('/hpc2hdd/home/testuser06/360Seg/DPPASS/dataset/adaption/s2d3d_pin_list/semantic_labels.json') as f:
            id2name = [name.split('_')[0] for name in json.load(f)] + ['<UNK>']
        with open('/hpc2hdd/home/testuser06/360Seg/DPPASS/dataset/adaption/s2d3d_pin_list/name2label.json') as f:
            name2id = json.load(f)
        self.colors = np.load('/hpc2hdd/home/testuser06/360Seg/DPPASS/dataset/adaption/s2d3d_pin_list/colors.npy')
